...
public:
	...
	TESWorldSpace* GetWorldSpace() { return ThisCall<TESWorldSpace*>(0x4E8880, this); }
	void MoveTo(TESObjectREFR* target, float offsetX = 0.0, float offsetY = 0.0, float offsetZ = 0.0) { CdeclCall<void>(0x528730, this, target, offsetX, offsetY, offsetZ); }
	TESObjectREFR* PlaceAtMe(TESForm* form, int count = 1, int distance = 0, int direction = 0, float healthPct = 1.0) { return CdeclCall<TESObjectREFR*>(0x539280, this, form, count, distance, direction, healthPct); }
};
...
enum ACTOR_AGGRESSION : UInt32
{
	ACTOR_AGGRESSION_UNAGGRESSIVE = 0x0,
	ACTOR_AGGRESSION_AGGRESSIVE = 0x1,
	ACTOR_AGGRESSION_VERY_AGGRESSIVE = 0x2,
	ACTOR_AGGRESSION_FRENZIED = 0x3,
	ACTOR_AGGRESSION_MAX = 0x3,
};

enum FactionRelation
{
	kFactionRelation_Neutral = 0x0,
	kFactionRelation_Enemy = 0x1,
	kFactionRelation_Ally = 0x2,
	kFactionRelation_Friend = 0x3,
};
...
public:
	...
	virtual void*		GetPreferredWeapon(int weaponType);
	...
	bool PlayerCommitedCrime() { return ThisCall<bool>(0x703B50, this); }
	bool InCombatAgainst(Actor* target) { return ThisCall<bool>(0x703950, this, target); }
	ACTOR_AGGRESSION GetCachedAggression() { return ThisCall<ACTOR_AGGRESSION>(0x6FA090, this); }
	FactionRelation GetFactionRelation(Actor* target, UInt8* out) { return ThisCall<FactionRelation>(0x70B780, this, target, out); }
	bool IsPassive();
	void SetPassive(bool isPassive);
	bool IsSneaking() { return ThisCall<bool>(0x45C200, this); }
	float GetRefVarData(const char* name);
	float GetInventoryWeight() { return (float)ThisCall<double>(0x4E8C70, this); }
	float GetCalculatedCarryWeight() { return (float)ThisCall<double>(0x6F6BC0, this);	}
};
...
public:
	...
	UInt8								unk72C[(0x900 - 0x72C)];		// 72C
	tList<CompassTarget>				compassTargets;					// 900
	UInt8								unk908[(0x9A4 - 0x908)];		// 61C
	bool								pcInCombat;						// 9A4
	bool								pcUnseen;						// 9A5
	UInt8								unk9A6[(0x9BC - 0x9A6)];		// 9A6
	bool IsHostileCompassTarget(Actor* actor);
...
struct NiPoint3
{
	...
	__inline float dot(const NiPoint3* pt)
	{
	...
	void Print()
	{
		Console_Print("x: %.2f, y: %.2f, z: %.2f", x, y, z);
	}
};
